...
<html>
	...
	<body>
		...
		<h1><center>List Event</center></h1>
		<br>
			<center><form name="form" action="fungsiaddkelas.php" method="post">
				...
				?>
						<tr>
							<td><?=$row['nama'];?></td>
							<td><?=$row['des_singkat'];?></td>
							<td><?=$row['des_lengkap'];?></td>
							<td><?=$row['link'];?></td>
							<td><?=$row['foto'];?></td>
							<td align="center"><a href= "editevent.php?id_event=<?php print($row['id_event']) ?>" > Edit</a>
								&nbsp;&nbsp;<a href= "deleteevent.php?id_event=<?php print($row['id_event']) ?>" > Hapus</a>
							</td>
					<?php
					}
					?>
