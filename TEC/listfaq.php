	...
	<body>
			...
			<center>
				...
				?>
						<tr>
							...
							<td><?=$row['jawaban'];?></td>
							<td align="center"><a href= "editfaq.php?id_faq=<?php print($row['id_faq']) ?>" > Edit</a>
								&nbsp;&nbsp;<a href= "deletefaq.php?id_faq=<?php print($row['id_faq']) ?>" > Hapus</a>
							</td>
					<?php
					}
					?>
		...
		<?php
			for($i=1;$i<=$total_nav;$i++) {
					if($i==1) echo '<a href="?p=listfaq&no=' . $i . '">First</a>';
					if($i==1) echo '<a href="?p=listfaq&no=' . ($no-1) . '">Previous</a>';
					echo '<a href="?p=listfaq&no=' . $i . '" ' . ($no == $i ? 'class="select"' : '') . '>' . $i . '</a>';
					if($i == $total_nav) echo '<a href="?p=listfaq&no=' . ($no+1) . '">Next</a>';
					if($i==$total_nav) echo '<a href="?p=listfaq&no=' . $i . '">Last</a>';
				}
				echo "<br/>";
