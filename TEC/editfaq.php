...
<html>
	...
	<body>
		...
		<h1><center>Edit FaQ</center></h1>
		<?php
			$id_faq = $_GET['id_faq'];
			$query2 = mysqli_query($conn, "select * from faq where id_faq='$id_faq'");
			$row2 = mysqli_fetch_array($query2);
		?>
		<br>
		<br>
		<form name="formdata" action="fungsieditfaq.php?id_faq=<?php print($id_faq); ?>" method="post" enctype="multipart/form-data">
			<center>
				<table width="500" height="300" border="0">
					<tr>
						<td>ID FaQ</td>
						<td>
							<?php print($row2['id_faq']); ?>
						</td>
					</tr>
					<tr>
						<td>pertanyaan</td>
						<td>
							<textarea name="pertanyaan" id="pertanyaan" cols="35" rows="5"><?php print($row2['pertanyaan']); ?></textarea>
						</td>
					</tr>
					<tr>
						<td>Jawaban</td>
						<td>
							<textarea name="jawaban" id="jawaban" cols="35" rows="8"><?php print($row2['jawaban']); ?></textarea>
						</td>
					</tr>
					<tr>
